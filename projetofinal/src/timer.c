...
void delay(unsigned int mSec){
	...
	HWREG(SOC_DMTIMER_7_REGS+IRQENABLE_SET) = 0x2; 
	...
	HWREG(SOC_DMTIMER_7_REGS+IRQENABLE_CLR) = 0x2; 
}
#endif


void timerEnable(unsigned int baseAdd){
    /* Wait for previous write to complete in TCLR */
	DMTimerWaitForWrite(0x1, baseAdd);
    /* Start the timer */
    HWREG(SOC_DMTIMER_7_REGS+DMTIMER_TCLR) |= DMTIMER_TCLR_ST;
}

void timerDisable(unsigned int baseAdd){
    /* Wait for previous write to complete in TCLR */
	DMTimerWaitForWrite(0x1, baseAdd);
    /* Stop the timer */
    HWREG(SOC_DMTIMER_7_REGS+DMTIMER_TCLR) &= ~DMTIMER_TCLR_ST;
...
void timerSetup(void){
    ...
    HWREG(CM_PER+CM_PER_TIMER7_CLKCTRL) |= 0x2;

	/*  Check clock enable for DMTIMER7 TRM 8.1.12.1.25 */    
    while((HWREG(CM_PER+CM_PER_TIMER7_CLKCTRL) & 0x3) != 0x2);

    /* Interrupt mask */
    HWREG(INTCPS+INTC_MIR_CLEAR2) |= (1<<31);//(95 --> Bit 31 do 3º registrador (MIR CLEAR2))
}

void timerSetFlag(bool value){
    flag_timer = value;
}
